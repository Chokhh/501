...
svm = GridSearchCV(LinearSVC(),param_grid,refit=True,verbose=2)
...
####  Feature importance  ####
imps1=permutation_importance(svm, X_test, y_test)
SVM_importances = imps1.importances_mean
std1 = imps1.importances_std
indices = np.argsort(SVM_importances)[::-1]
feature_names=X_train.columns
feat_importances = pd.Series(SVM_importances, index=feature_names)
...
##################()
param_grid = {'C': [0.01,0.1,1,10,100]}
svm = GridSearchCV(SVC(kernel='poly'),param_grid,refit=True,verbose=2)
svm.fit(X_train,y_train)
print(svm.best_estimator_)
predicted = svm.predict(X_test)
print(classification_report(y_test,predicted))
...
##################
## SVM kernel 2 ## sigmoid
##################()
param_grid = {'C': [0.01,0.1,1,10,100]}
svm = GridSearchCV(SVC(kernel='sigmoid'),param_grid,refit=True,verbose=2)
svm.fit(X_train,y_train)
print(svm.best_estimator_)
predicted = svm.predict(X_test)
print(classification_report(y_test,predicted))
